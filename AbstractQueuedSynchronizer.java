...
public abstract class AbstractQueuedSynchronizer
     ...
     */
    private Node enq(final Node node) { //把参数节点插入等待队列
        for (;;) {
            Node t = tail;
	...
	//找到等的最久的节点即头节点调度，可能不是属于当前线程，会在tryAcquire函数内直接返回false
                    setHead(node);
                    p.next = null; // help GC
                    failed = false;
	 ...
	 * 排他模式获取，无视中断，成功后返回，否则放入队列，等待再次调度直至成功
     *
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&	//先尝试获取，失败的话加入等待队列，然后调度队列中的本线程。
			//这是本线程是在尾部的
